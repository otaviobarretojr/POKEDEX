...
@Composable fun PokedexApp(){
 ...
 Scaffold(topBar={if(!isSecondaryScreen&&!useCompactOwnHeader)TopAppBar(title={Text("POKEDEX")})},bottomBar={if(!isSecondaryScreen){NavigationBar{mainDestinations.forEach{d->NavigationBarItem(selected=currentRoute==d.route,onClick={navController.navigate(d.route){popUpTo(navController.graph.findStartDestination().id){saveState=true};launchSingleTop=true;restoreState=true}},icon={Icon(d.icon,d.label)},label={Text(d.label)})}}}}){innerPadding->
  NavHost(navController,"pokedex",Modifier.padding(innerPadding)){
   ...
   composable("regionExplorer?source={source}",arguments=listOf(navArgument("source"){type=NavType.StringType;nullable=false})){entry->val source=entry.arguments?.getString("source")?.let(Uri::decode).orEmpty();RegionExplorerV4Screen(source,{navController.popBackStack()},{id,gameSource->openPokemon(id,gameSource)})}
