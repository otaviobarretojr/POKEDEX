...
@Composable fun PokedexApp(){
 ...
 Scaffold(containerColor=MaterialTheme.colorScheme.background,bottomBar={if(!isSecondaryScreen){DexBottomBar(mainDestinations,currentRoute){d->if(d.route!=currentRoute)haptics.performHapticFeedback(HapticFeedbackType.TextHandleMove);navController.navigate(d.route){popUpTo(PokedexRoutes.HOME){saveState=true};launchSingleTop=true;restoreState=true}}}}){innerPadding->
  NavHost(navController,PokedexRoutes.HOME,Modifier.padding(innerPadding),enterTransition={fadeIn(tween(PokedexDesignTokens.Motion.Standard))+slideInHorizontally(tween(PokedexDesignTokens.Motion.Screen)){it/10}},exitTransition={fadeOut(tween(PokedexDesignTokens.Motion.Fast))+slideOutHorizontally(tween(PokedexDesignTokens.Motion.Screen)){-(it/14)}},popEnterTransition={fadeIn(tween(PokedexDesignTokens.Motion.Standard))+slideInHorizontally(tween(PokedexDesignTokens.Motion.Screen)){-(it/10)}},popExitTransition={fadeOut(tween(PokedexDesignTokens.Motion.Fast))+slideOutHorizontally(tween(PokedexDesignTokens.Motion.Screen)){it/14}}){
