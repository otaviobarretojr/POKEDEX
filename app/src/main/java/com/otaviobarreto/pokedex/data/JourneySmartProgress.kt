...
object JourneySmartProgress {
    fun contextForStep(game:String,stepId:String):JourneySmartContext{
        val steps=JourneyCatalog.steps(game)
        val target=steps.firstOrNull{it.id==stepId} ?: return context(game)
        val index=steps.indexOf(target).coerceAtLeast(0)
        ...
        val mainIndex=mainStory.indexOf(target)
        val mainRatio=when{
            mainStory.isEmpty() -> 0f
            target.kind in setOf(JourneyChallengeKind.POSTGAME,JourneyChallengeKind.DLC,JourneyChallengeKind.EPILOGUE) -> 1f
            mainIndex<0 -> 1f
            else -> mainIndex.toFloat()/mainStory.size.coerceAtLeast(1)
        }
        val phase=when{
            target.kind in setOf(JourneyChallengeKind.POSTGAME,JourneyChallengeKind.DLC,JourneyChallengeKind.EPILOGUE) -> CampaignPhase.LATE
            mainRatio < .34f -> CampaignPhase.EARLY
            mainRatio < .72f -> CampaignPhase.MID
            else -> CampaignPhase.LATE
        }
        val label=phaseLabelFor(target.id,phase)
        return JourneySmartContext(
            completedCount=index,
            totalCount=steps.size,
            nextStep=target,
            phase=phase,
            phaseLabel=label,
            recommendation="Foco selecionado: "+target.title+" · "+target.levelLabel+". Etapa: "+label+"."
        )
    }

    private fun phaseLabelFor(nextId:String,phase:CampaignPhase):String=when{
    ...
    fun context(game:String):JourneySmartContext{
        ...
        val label=phaseLabelFor(nextId,phase)
