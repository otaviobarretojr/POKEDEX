...
@Composable fun PokedexApp(){
 ...
 val isSecondaryScreen=currentRoute=="pokemon/{id}?source={source}"||currentRoute=="gameDex?source={source}"||currentRoute=="location/{id}?source={source}"||currentRoute=="regionMap/{id}?source={source}"||currentRoute=="regionExplorer?source={source}"||currentRoute=="reference?kind={kind}&name={name}"||currentRoute=="campaignGuide?game={game}";val isMainDestination=currentRoute in mainDestinations.map{it.route};val useCompactOwnHeader=currentRoute=="boxes"
 ...
 Scaffold(bottomBar={if(!isSecondaryScreen){NavigationBar{mainDestinations.forEach{d->NavigationBarItem(selected=currentRoute==d.route,onClick={navController.navigate(d.route){popUpTo("home"){saveState=true};launchSingleTop=true;restoreState=true}},icon={Icon(d.icon,d.label)},label={Text(d.label)},alwaysShowLabel=false)}}}},topBar={if(isMainDestination&&currentRoute!="home"){TopAppBar(title={Text(mainDestinations.firstOrNull{it.route==currentRoute}?.label?:"POKEDEX")},navigationIcon={IconButton(onClick={navController.navigate("home"){popUpTo("home"){inclusive=false};launchSingleTop=true}}){Icon(Icons.Default.Home,"Voltar ao início")}})}}){innerPadding->
  NavHost(navController,"home",Modifier.padding(innerPadding)){
   composable("home"){JourneyScreen(onPokemonClick={openPokemon(it)},onOpenTeamGuide=::openCampaignGuide,onOpenGameDex=::openGameDex)}
   composable("campaignGuide?game={game}",arguments=listOf(navArgument("game"){type=NavType.StringType;nullable=false})){entry->val game=entry.arguments?.getString("game")?.let(Uri::decode);CampaignTeamGuideScreen(onBackToMyTeams={navController.popBackStack()},onPokemonClick={openPokemon(it)},initialGame=game)}
