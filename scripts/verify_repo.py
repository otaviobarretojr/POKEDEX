...
if "PACK_VERSION = 20" not in offline:
    violations.append("Offline pack version is not v20")

app = (root / "app/src/main/java/com/otaviobarreto/pokedex/PokedexApplication.kt").read_text(encoding="utf-8")
if ".crossfade(false)" not in app or "1536L * 1024L * 1024L" not in app:
...
if "21307" not in workflow or "20.13.7" not in workflow:
    violations.append("CI v20.13.7 version validation missing")
...
for required in ('"Legends Arceus" -> legendsArceus', '"la-01"', '"la-07"', '"la-18"', '"la-27"', '"la-db-95"', '"la-db-102"'):
    if required not in journey_catalog:
...
for required in ('"la-07"', '"la-12"', '"la-26"', '"la-27"', '"la-db-95"', '"la-db-102"'):
    if required not in journey_preparation:
...
for required in ('"Sword / Shield" -> swordShield', '"swsh-g1"', '"swsh-g8"', '"swsh-15"', '"swsh-pg-02"', '"swsh-ioa-04"', '"swsh-ct-05"'):
    if required not in journey_catalog:
        violations.append(f"Sword Shield Journey route missing {required}")

for required in ("stepId.startsWith(\"swsh-\")", "galarCatchPlan", "GYM CHALLENGE · PRIMEIRAS BADGES", "DLC · ISLE OF ARMOR", "DLC · CROWN TUNDRA"):
    if required not in journey_team_progress:
        violations.append(f"Sword Shield dynamic progression missing {required}")

for required in ('"swsh-g1"', '"swsh-g8"', '"swsh-15"', '"swsh-ioa-04"', '"swsh-ct-05"'):
    if required not in journey_preparation:
        violations.append(f"Sword Shield preparation coverage missing {required}")

for required in ('"Sword / Shield" -> galar', '"Scorbunny"', '"Grookey"', '"Sobble"'):
    if required not in starter_catalog:
...
for required in ("Kleavor","Hisuian Lilligant","Hisuian Arcanine","Hisuian Electrode","Hisuian Avalugg","Dialga / Palkia","Heatran","Cresselia","Regigigas","Giratina","Arceus","Daybreak"):
    if required not in visual_catalog:
...
for required in ("PACK_VERSION = 20", "cachedImages", "expectedImages", "hasOfflineArtwork", 'openSnapshot("pokemon-offline-$id")'):
...
local_v1901 = 'versionName = "19.0.1"' in local_gradle and "versionCode = 19001" in local_gradle
...
for required in ("GameCoverCatalog.coversFor", "activeJourneyArtworkUrls", "Aquecendo sua Jornada"):
...
for required in ("JourneyGameVisualCatalog.forGame", "activeJourneyArtworkUrls", "Aquecendo sua Jornada"):
...
for required in ("PACK_VERSION = 20", "formArtworkKey(", '"shiny" else "normal"', "countsForLivingDex"):
...
if "PACK_VERSION = 20" not in offline_v13:
...
if "PACK_VERSION = 20" not in offline_v14:
...
if "PACK_VERSION = 20" not in offline_v15:
...
if "PACK_VERSION = 20" not in offline_v16:
    violations.append("v20 offline pack version missing")
...
if "Estado atual — v20.13.7" not in readme_v1810:
    violations.append("README current version is not v20.13.7")
...
integrity_v1830 = (root / "app/src/main/java/com/otaviobarreto/pokedex/data/CollectionIntegrityService.kt").read_text(encoding="utf-8")
for required in ("CollectionIntegrityReport", "missingGlobalOwnership", "missingContextualOwnership", "fun repair()"):
    if required not in integrity_v1830:
        violations.append(f"v18.3 collection integrity guard missing {required}")

offline_v1830 = (root / "app/src/main/java/com/otaviobarreto/pokedex/data/OfflineGamePackManager.kt").read_text(encoding="utf-8")
if "PACK_VERSION = 20" not in offline_v1830:
...
for required in ("MAX_UNPINNED_BYTES", "MAX_UNPINNED_FILES", "pruneUnpinned", "pinnedCount"):
    if required not in cache_v1830:
        violations.append(f"v18.3 bounded persistent cache missing {required}")

settings_v1830 = (ui / "CompanionCenterScreen.kt").read_text(encoding="utf-8")
for required in ("Verificar integridade", "PersistentApiCache.sizeBytes()", "StartupPreloader.lastWarmDurationMs"):
    if required not in settings_v1830:
        violations.append(f"v18.3 diagnostics UI missing {required}")

application_v1830 = (root / "app/src/main/java/com/otaviobarreto/pokedex/PokedexApplication.kt").read_text(encoding="utf-8")
if "CollectionIntegrityService.repair()" not in application_v1830:
    violations.append("v18.3 startup integrity repair missing")

root_gradle_v1830 = (root / "build.gradle.kts").read_text(encoding="utf-8")
if 'com.android.application") version "8.6.1"' not in root_gradle_v1830:
    violations.append("v18.3 AGP 8.6.1 toolchain missing")
